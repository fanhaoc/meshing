...
public:
	...
	//遍历网格的顶点
	std::vector<aiVector3D> processVertex(aiMesh* mesh) {
	...
	// 遍历网格的法线
	std::vector<aiVector3D> processNormal(aiMesh* mesh) {
		std::vector<aiVector3D> data;
		for (unsigned int i = 0; i < mesh->mNumVertices; ++i) {
	...
	//遍历网格的面
	std::vector<aiFace> processFace(aiMesh* mesh) {
		std::vector<aiFace> data;
